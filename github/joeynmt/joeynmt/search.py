...
def beam_search(
        ...
        embed: Embeddings, n_best: int = 50) -> (np.array, np.array):
    """
    ...
    for step in range(max_output_length):
        ...
        #sy debug
        #print("decoder.output_size.size():")
        #print(decoder.output_size)


        #sy debug
        #print("curr_scores.size() after flattening:")
        #print(curr_scores.size())
    ...
    def pad_and_stack_hyps(hyps, pad_value):
        #print("hyps: ",type(hyps))
        #print("length of hyps:", len(hyps))
        #print("hyps[0].shape:",hyps[0].shape)
    ...
    for r in results["predictions"]:
       for j in range(len(r)):
            full_n_best.append(r[j].cpu().numpy())

    for r_s in results["scores"]:
        for j in range(len(r_s)):
            n_best_score.append(r_s[j].cpu().numpy())

    #print(len(results["predictions"]))
    #print(len(results["predictions"][0]))
    #print("length of full_n_best: ",len(full_n_best))
    #print("length of n_best_score: ", len(n_best_score))


    #print("type of pad_index:",type(pad_index))
    #print(pad_index)

    final_outputs_n_best = pad_and_stack_hyps(full_n_best,
                                       pad_value=pad_index)

    #print("final_outputs_n_best: ",final_outputs_n_best.shape)
    #print("final_outputs: ", final_outputs.shape)

    #sy_debug
    #print("results.score: ", results[])
    #print("results:")
    #print("number of elements in list prediction: ", len(results["predictions"]))
    #print("number of elements in list prediction[0]: ", len(results["predictions"][0]))
    #print("type of prediction[0]:",type(results["predictions"][0]))
    #print("type of prediction[0][0]",type(results["predictions"][0][0]))
    #print("size of predictions[0][0]: ",results["predictions"][0][0].size())
    #print("predictions: ",results["predictions"])
    #print("predictions[0]:",results["predictions"][0])
    #print("predictions[0][0]:",results["predictions"][0][0])

    #print(type(results["scores"][0][0]))
    #print(results["scores"][0][0].size())
    #print(results["scores"][0][0])
    #print(type(results["gold_score"][0]))
    #print(results["gold_score"][0])
    ...
    return final_outputs_n_best, n_best_score, None
    #return final_outputs_n_best, None

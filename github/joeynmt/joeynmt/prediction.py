...
def validate_on_data(model: Model, data: Dataset,
    ...
    with torch.no_grad():
        ...
        for valid_batch in iter(valid_iter):
            ...
            output, n_best_score, attention_scores = model.run_batch(
            ...
            #sy_debug
            scores.extend(n_best_score)
            all_outputs.extend(output)
            valid_attention_scores.extend(
                attention_scores if attention_scores is not None else [])
        ...
        decoded_valid = model.trg_vocab.arrays_to_sentences(arrays=all_outputs,
                                                            cut_at_eos=True)
        #sy_debug
        best_hyp = [decoded_valid[h] for h in range(0,len(decoded_valid),50)]  # step = n_best, use for calculating bleu scores only

        # evaluate with metric on full dataset
        join_char = " " if level in ["word", "bpe"] else ""
        valid_sources = [join_char.join(s) for s in data.src]
        valid_references = [join_char.join(t) for t in data.trg]
        valid_hypotheses_full_n_best = [join_char.join(t) for t in decoded_valid]
        valid_hypotheses = [join_char.join(t) for t in best_hyp]
        ...
        if level == "bpe":
            ...
            valid_hypotheses_full_n_best= [bpe_postprocess(v) for
                                v in valid_hypotheses_full_n_best] #sy_debug n_best
    ...
    return current_valid_score, valid_loss, valid_ppl, valid_sources, \
        ...
        decoded_valid, valid_attention_scores, valid_hypotheses_full_n_best,scores
            ...
            def write_to_csv(hyps:list, scores:list):
                ...
                with open(output_file, mode="w", newline='',encoding="utf-8") as out_file:
                    ...
                    for prediction, score in zip(hyps,scores):
                        writer.writerow({fieldnames[0]:prediction, fieldnames[1]:score})
